...
.\openssl\openssl.exe pkcs12 -in $pfxPath -nocerts -out $keyPath -nodes -passin pass:$password
.\openssl\openssl.exe pkcs12 -in $pfxPath -nokeys -out $certPath -nodes -passin pass:$password
